...
#let hyväksyttävätTyönTyypit = (
  kandidaatinTyönTyyppi,
  diplomiTyönTyyppi,
  maisterinTyönTyyppi,
  lisensiaatinTyönTyyppi,
  tohtorinTyönTyyppi,
)
...
#let thesisTypeValidationFn(thesisType, työnTyyppi, language, includeFinnishAbstract) = {
  ...
  let thesisTypeLevelsMatchPredicate = (
    (thesisType == bachelorsThesisType) and (työnTyyppi == kandidaatinTyönTyyppi)
      or (thesisType == mastersThesisType) and (työnTyyppi in (diplomiTyönTyyppi, maisterinTyönTyyppi))
      or (thesisType == licentiateThesisType) and (työnTyyppi == lisensiaatinTyönTyyppi)
      or (thesisType == doctoralThesisType) and (työnTyyppi == tohtorinTyönTyyppi)
  )
...
#let array2SectionNumbering(arr, inAppendix) = {
  if arr.len() < 1 {
    panic(
      "Arrays of zero length cannot be used to construct a section number string. Reveived " + arr.map(str).join(","),
    )
  }
  if not (arr.all(elem => type(elem) == int)) {
    panic("Array elements used to construct section numberings need to contain integers.")
  }
    ...
    let refText = if el.func() == figure {
      let (carray, elementSupplement) = if el.kind == image {
        ...
        let figuresBefore = query(
          selector(figure).before(elementLocation),
        )
        let parentFigure = figuresBefore.filter(x => x.kind != SUBFIGUREKIND).last()
        let parentCounter = figureCounterFromKind(parentFigure.kind)
        let parentCounterValue = parentCounter.at(parentFigure.location())
        (
          truncatedHeadingCounter + parentCounterValue + SUBFIGURECOUNTER.at(elementLocation),
      ...
      carray.last() += 1
      // If our element is a subfigure, also manually increment the parent counter.
      if el.kind == SUBFIGUREKIND {
        carray.at(-2) += 1
      }
      let sliceEnd = if el.kind == SUBFIGUREKIND {
        -1
      } else {
        carray.len()
      }
...
#let figShowRule(language, figNumberWithinLevel, it) = {
  ...
  let ctr = if it.kind == image {
    headingCounter + IMAGECOUNTER.get()
  } else if it.kind == table {
    headingCounter + TABLECOUNTER.get()
  } else if it.kind == raw {
    headingCounter + CODECOUNTER.get()
  } else if it.kind == TAUTHEOREMKIND {
    headingCounter + TAUTHEOREMCOUNTER.get()
  } else if it.kind == PUBLICATIONKIND {
    PUBLICATIONCOUNTER.get()
  } else if it.kind == SUBFIGUREKIND {
    SUBFIGURECOUNTER.get()
  } else {
    headingCounter + UNKNOWNCOUNTER.get()
  }
      ...
      if it.level == 1 and inAppendix {
        ...
        + if it.outlined {
          ...
          if inPublications {
            (
              counter(heading).display("1.1")
                + for _ in range(calc.max(2, 3 - it.level)) {
                  sym.space.nobreak
                }
            )
          }
...
#let outlineEntryShowRule(
  ...
  let elementPageNum = (
    MAINMATTERPAGENUMBERCOUNTER.at(elementLocation).first()
      + if thesisTypeNum < licentiateThesisTypeInt { -1 } else { 0 }
  )
  let headingCounter = counter(heading).at(elementLocation)
  let headingCounter = headingCounter.slice(
    0,
  ...
  let (number, space, entry) = if el.func() == figure {
    ...
    panic(
      "Lists of contents may only reference headings or figures. How we got here is a mystery (and probably a bug in the template).",
    )
  }
  (
    v(0.75cm, weak: true)
...
#let contentFigures(language) = {
  outline(
    title: if language == finnish [Kuvaluettelo] else if language == english [List of figures],
...
#let contentTables(language) = {
  outline(
    title: if language == finnish [Taulukkoluettelo] else if language == english [List of tables],
...
#let contentListings(language) = {
  outline(
    title: if language == finnish [Listausluettelo] else if language == english [List of listings],
...
#let template(
  ...
  set document(
    ...
    description: description
      ...
      + examiners
        .map(
          ex => (ex.title, ex.firstname, ex.lastname).join(" "),
        )
        .join(", ", last: " and "),
  )
...
#let displayPublication(citeKey, publication, language) = {
  context page(margin: (x: 1cm, y: 1cm), header: [], footer: [])[
    ...
    #block[
      ...
      #if type(publication.author) == str { publication.author } else if type(publication.author) == array {
        if publication.author.len() > 10 {
          publication.author.first()
          " et al."
        } else { publication.author.join(", ", last: " and ") }
      } else { panic("Author field of a publication " + citeKey + " needs to be given as a string or an array!") },
      #if lower(publication.type) == "article" {
